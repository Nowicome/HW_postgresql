...
SELECT name_of_the_track, duration
FROM tracks
WHERE lower(name_of_the_track) LIKE '%мой%' OR lower(name_of_the_track) LIKE '%my%';
...
SELECT title, count(name_of_the_track) n FROM albums a
...
WHERE NOT EXTRACT (YEAR FROM a.year_of_issue) = 2020
GROUP BY p.name_or_alias
...
SELECT DISTINCT title FROM albums a
...
GROUP BY a.title
HAVING count(DISTINCT g.name_of_the_genre) > 1
ORDER BY a.title
